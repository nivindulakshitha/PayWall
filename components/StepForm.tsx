    ...
    switch (step) {
      ...
      case 3: // Location/Distance OR Bank Details
        if (inputs.method === 'online') {
          return (
            <motion.div variants={itemVariants} className="space-y-4">
              <p className="text-sm text-gray-400">{t('bank.title')}</p>
              <div 
                className="glass-card-elevated !p-6 cursor-pointer group hover:border-indigo-500/30 transition-all"
                onClick={() => {
                  const details = `${t('bank.account')}\n${t('bank.bank')}\n${t('bank.branch')}\n${t('bank.holder')}`;
              ...
              >
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    <p className="text-sm font-bold text-indigo-400 uppercase tracking-widest">Bank of Ceylon</p>
                    <svg className="w-5 h-5 text-indigo-500/50 group-hover:text-indigo-400 transition-colors" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 5H6a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2v-1M8 5a2 2 0 002 2h2a2 2 0 002-2M8 5a2 2 0 012-2h2a2 2 0 012 2m0 0h2a2 2 0 012 2v3m2 4H10m0 0l3-3m-3 3l3 3" />
                    </svg>
                  </div>
                  <div className="space-y-1">
                    <p className="text-lg font-bold text-white tracking-tight">{t('bank.account')}</p>
                    <p className="text-sm text-gray-400">{t('bank.holder')}</p>
                    <p className="text-xs text-gray-500 uppercase">{t('bank.branch')}</p>
                  </div>
                </div>
                <div className="mt-4 pt-4 border-t border-white/5 text-center">
                  <p className="text-[10px] text-indigo-400/70 font-bold uppercase tracking-widest group-hover:text-indigo-400 transition-colors">
                    {t('bank.copy')}
                  </p>
                </div>
              </div>
